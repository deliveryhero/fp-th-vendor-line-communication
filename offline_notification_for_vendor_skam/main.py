...
query_table = "foodpanda-th-bigquery.pandata_th.offline_notification_for_vendor_SKAM"
...
try:
  dataframe = query_BQ_table(query)
    ...
    json = {'text' : '*offline_notification_for_vendor_skam*: Failed to get data: ' + str(e)})
...
try:
  reponse_code_list, json_list = send_request_line_api_v6(url = url, headers = headers,
    ...
    requests.post(slack_webhook,
    json = {'text' : '*offline_notification_for_vendor_skam*: Failed send API request: ' + str(e)})
...
dataframe1["template_id_if_any"] = "offline_notification_for_vendor_skam"
...
try:
  status = record_line_communication_logs(logs_table_id, df_records)
    ...
    requests.post(slack_webhook,
    json = {'text' : '*offline_notification_for_vendor_skam*: Failed to record logs: ' + str(e)})
