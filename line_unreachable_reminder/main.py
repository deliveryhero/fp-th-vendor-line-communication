...
if Live == False:
    query = f"""
      ...
      WHERE line_user_id IS NOT NULL
        AND DATE(record_created_at_local) = CURRENT_DATE("Asia/Bangkok")
      LIMIT 1
    """
...
try:
  ...
  json = {'text' : '*line_unreachable_reminder*: Failed to get data: ' + str(e)})
...
try:
  ...
  requests.post(slack_webhook,
  json = {'text' : '*line_unreachable_reminder*: Failed send API request: ' + str(e)})

try:
  df = dataframe.filter(items=['vendor_code', 'line_user_id'])
  df["return_response"] = reponse_code_list
  df["msg_sent_date_time"] = now
  df["template_id_if_any"] = "line_unreachable_reminder"
  df["msg_url"] = url
  df["msg_content"] = "line_unreachable_reminder"
  df_records = df.to_dict('records')
  status = record_line_communication_logs(logs_table_id, df_records)
except BaseException as e:
  requests.post(slack_webhook,
  json = {'text' : '*line_unreachable_reminder*: Failed to record logs: ' + str(e)})
  # print(e)
