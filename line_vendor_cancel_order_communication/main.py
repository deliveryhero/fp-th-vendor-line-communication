...
from send_line_message import send_request_line_api_cancel_order_communication
...
if Live == False:
    ...
    FROM `fulfillment-dwh-production.pandata_report.country_TH_vendor_experience_auto_cancel_order_communication` AS rider_late
...
try:
  ...
  json = {'text' : '*line_vendor_cancel_order_communication*: Failed to get data: ' + str(e)})
  # print("Cannot create dataframe")
  # print(e)

try:
  reponse_code_list, json_list = send_request_line_api_cancel_order_communication(url = url,
                                                          headers = headers,
                                                          json_object = json_object,
                                                          dataframe = dataframe)
except BaseException as e:
  requests.post(slack_webhook,
  json = {'text' : '*line_vendor_cancel_order_communication*: Failed send API request: ' + str(e)})

df = dataframe.filter(items=['vendor_code', 'line_user_id'])
df["return_response"] = reponse_code_list
df["msg_sent_date_time"] = now
df["template_id_if_any"] = "line_vendor_cancel_order_communication"
df["msg_url"] = url
df["msg_content"] = "line_vendor_cancel_order_communication"
...
try:
  ...
  requests.post(slack_webhook,
  json = {'text' : '*line_vendor_cancel_order_communication*: Failed to record logs: ' + str(e)})
  # print(e)
